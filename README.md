...
`import_bulk`'s per-batch overhead plus the per-edge document lookup
mentioned above, but it's worth re-checking rather than taking on faith —
it's a real gap in this run, not a rounding error.

Cypher and AQL also aren't a perfectly fair fight to begin with. The
traversal queries use each language's own idiomatic syntax rather than a
literal word-for-word translation, so part of the gap between ArangoDB and
the rest is really a difference in query planners, not just engines.

Footprint numbers only came through for three of the five platforms.
CognoDB and AuraDB simply don't expose memory or storage figures through
any query-level API on their free tiers — I checked several ways before
giving up, so that's a confirmed dead end, not something I didn't get
around to.

And last: this was one run, on one day, from one machine. No repeats, no
variance numbers, no cold-start comparison beyond the warm-up already
built into each measurement. Take it as a solid first pass, not a final
verdict — the whole point of publishing the harness alongside it is so
anyone can rerun it and check.
